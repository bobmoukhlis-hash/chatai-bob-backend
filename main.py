...
class ImageRequest(BaseModel):
    prompt: str
    client_id: str
    style: Optional[str] = "cartoon"   # <-- AGGIUNTO
...
def set_premium(email: str = Form(...), key: str = Form(...)):
    if key != (os.getenv("ADMIN_KEY") or ""):
        return {"ok": False}
    DB.execute("UPDATE users SET premium=1 WHERE email=?", (email.lower(),))
    DB.commit()
    return {"ok": True}
...
def image(req: ImageRequest, authorization: Optional[str] = Header(None)):
    ...
    try:
        img = pollinations_image(final_prompt)
...
def video(req: VideoRequest, authorization: Optional[str] = Header(None)):
    ...
    style = (req.style or "cartoon")
    base = f"{style_prompt(style)}, same character, smooth motion"

    prompts = [f"{base}, {req.prompt}, scene {i}" for i in range(1, 5)]
